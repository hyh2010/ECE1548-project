...
class TrafficSourceLoadBalance(TrafficSourcePoissonArrival):
    ...
    def __load_balance(self):
        ...
        if (len(self.traffic()) >= self.n()):
            ...
            self.rho1_estimates()['x'].append(self.queue().env().now)
            self.rho1_estimates()['y'].append(rho1_estimate)
    ...
    def rho1_estimates(self):
        return self.__rho1_estimates

    def reassign(self):
        return self.__reassign

    def reassign_from_controller(self):
        return self.load_balance_controller

    def reassign_from_controller_unknown_mu(self):
        return self.load_balance_no_mu_controller
